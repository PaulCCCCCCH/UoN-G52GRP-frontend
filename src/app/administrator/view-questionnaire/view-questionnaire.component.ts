...
export class ViewQuestionnaireComponent implements OnInit {
   ...
   */
  viewResponseList() {
    this.router.navigate(['admin/view-response-list/' + this.id]);
  }
   ...
   */
  viewOverall() {
    this.router.navigate(['admin/questionnaire/view-overall/' + this.status + '/' + this.id]);
  }
  ...
  closeQuestionnaire() {
    this.questionnaireService.archiveQuestionnaire(this.id).subscribe(
      res => {
        ...
        this.modalService.dismissAll();
        this.router.navigate(['admin/questionnaires/archived']);
      },
