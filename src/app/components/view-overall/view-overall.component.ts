...
export class ViewOverallComponent implements OnInit {
  ...
  ngOnInit() {
    this.responseService.getResponseSets(this.id).subscribe(responseSets => {
      this.responseSets = responseSets;
      this.responseNumber = responseSets.length;
      this.questionService.getQuestionSet(this.id).subscribe(questionSet => {
